...
public class NoteControllerTest {
	...
	public void testCreateNote() throws Exception {
		...
		mockMvc.perform(post("/all/note/create").contentType(MediaType.APPLICATION_JSON).content(noteJson))
				.andExpect(status().isCreated()).andExpect(jsonPath("$.noteContent").value("testContent"))
				.andExpect(jsonPath("$.noteType").value("testType")).andExpect(jsonPath("$.weekNumber").value(1))
				.andExpect(jsonPath("$.batchId").value(1)).andExpect(jsonPath("$.traineeId").value(1));
	}
	...
	public void testFindAllNotes() throws Exception {
		...
		mockMvc.perform(get("/all/note/all").contentType(MediaType.APPLICATION_JSON).content(noteJson))
				.andExpect(status().isOk()).andExpect(jsonPath("$[0].noteContent").value("testContent"))
				.andExpect(jsonPath("$[0].noteType").value("testType")).andExpect(jsonPath("$[0].weekNumber").value(1))
				.andExpect(jsonPath("$[0].batchId").value(1)).andExpect(jsonPath("$[0].traineeId").value(1));
	}
	...
	public void testFindNotesByTrainee() throws Exception {
		...
		mockMvc.perform(get("/all/note/trainee/" + traineeId).contentType(MediaType.APPLICATION_JSON).content(noteJson))
				.andExpect(status().isOk()).andExpect(jsonPath("$[0].noteContent").value("testContent"))
				.andExpect(jsonPath("$[0].noteType").value("testType")).andExpect(jsonPath("$[0].weekNumber").value(1))
				.andExpect(jsonPath("$[0].batchId").value(1)).andExpect(jsonPath("$[0].traineeId").value(traineeId));
	}
	...
	public void testUpdateNote() throws Exception {
		...
		mockMvc.perform(put("/all/note/update").contentType(MediaType.APPLICATION_JSON).content(noteJson))
				.andExpect(status().isOk()).andExpect(jsonPath("$.noteContent").value("testContent"))
				.andExpect(jsonPath("$.noteType").value("testType")).andExpect(jsonPath("$.weekNumber").value(1))
				.andExpect(jsonPath("$.batchId").value(1)).andExpect(jsonPath("$.traineeId").value(1));
	}
	...
	public void testUpdateNoteFail() throws Exception {
		...
		when(noteServiceMock.updateNote(any(NoteDTO.class)))
				.thenThrow(new DoesNotExistException("Note does not already exist"));
	...
	public void testFindNotesByBatch() throws Exception {
		...
		mockMvc.perform(get("/all/note/batch/" + batchId).contentType(MediaType.APPLICATION_JSON).content(noteJson))
				.andExpect(status().isOk()).andExpect(jsonPath("$[0].noteContent").value("testContent"))
				.andExpect(jsonPath("$[0].noteType").value("testType")).andExpect(jsonPath("$[0].weekNumber").value(1))
				.andExpect(jsonPath("$[0].batchId").value(batchId)).andExpect(jsonPath("$[0].traineeId").value(1));
	}
	...
	public void testFindNotesById() throws Exception {
		...
		mockMvc.perform(get("/all/note/" + noteId).contentType(MediaType.APPLICATION_JSON).content(noteJson))
				.andExpect(status().isOk()).andExpect(jsonPath("$.noteContent").value("testContent"))
				.andExpect(jsonPath("$.noteType").value("testType")).andExpect(jsonPath("$.weekNumber").value(1))
				.andExpect(jsonPath("$.noteId").value(noteId)).andExpect(jsonPath("$.batchId").value(1))
